<?php
  ...
  */
  ...
  class LevelAPI
  {
    private $server, $levels, $default, $mapName, $mapDir, $time, $seed, $spawn, $level;
    ...
    public function unloadLevel(Level $level, $force = false)
    {
      ...
      $entitys = $this->server->api->entity->getAll($level);
    ...
    public function loadLevel($name)
    {
      ...
      foreach ($tilesA as $tile) {
        ...
        $this->server->api->tile->add($this->levels[$name], $tile["id"], $tile["x"], $tile["y"], $tile["z"], $tile);
      }

      //$timeu = microtime(true);
      $blocks=$blockUpdates->getAll();
      foreach ($blocks as $bupdate) {
