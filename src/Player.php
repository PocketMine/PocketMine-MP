      ...
      while ($count > 0) {
        ...
        foreach ($this->inventory as $s => $item) {
          if ($item->getID() === AIR) {
            ...
            $this->inventory[$s] = BlockAPI::getItem($type, $damage, $item->count + $add);
            break;
          }
        }
        if ($add === 0) {
      ...
      if (($this->gamemode & 0x01) === ($gm & 0x01)) {
        if (($gm & 0x01) === 0x01 and ($gm & 0x02) === 0x02) {
          ...
          foreach (BlockAPI::$creative as $item) {
            $this->inventory[] = BlockAPI::getItem(DANDELION, 0, 1);
          }
        } elseif (($gm & 0x01) === 0x01) {
          $this->inventory = array();
          foreach (BlockAPI::$creative as $item) {
            $this->inventory[] = BlockAPI::getItem($item[0], $item[1], 1);
          }
        }
        $this->gamemode = $gm;
    ...
    public function handleDataPacket($pid, $data)
    {
      switch ($pid) {
        ...
        case MC_PLAYER_ACTION:
          ...
          if ($this->entity->inAction === true) {
            //Shot arrow
            if ($data["action"] === 5 and $this->getSlot($this->slot)->getID() === BOW and $this->startAction !== false) {
              $d = array(
                "x" => $this->entity->x,
                "y" => $this->entity->y + 1.6,
                "z" => $this->entity->z,
